...
A **readers-writer lock** ([`std::sync::RwLock`](https://doc.rust-lang.org/std/sync/struct.RwLock.html)) is a synchronization primitive for multi-threaded shared mutable state that allows multiple readers OR one writer at a time. It is best used for scenarios where reads are frequent and writes are infrequent. It has two key methods:
