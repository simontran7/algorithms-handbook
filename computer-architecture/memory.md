...
2. **Check for a matching tag**
    ...
    If no match is found, or the valid bit is 0, we call this unsuccessful access a **cache miss**.
...
<img src="images/cache-hit.png" width="500">
...
5. **Perform the access (read or write)**
    ...
    On a **write hit** (i.e.,  the CPU wants to write to an address, and that block is already in the cache), the CPU updates the data in the cache line. What happens next depends on the **write-hit policy**:
    ...
    On a **write miss** (i.e., the CPU wants to write to an address, and that block is *not* in the cache), the behaviour depends on the **write-miss policy**:
    ...
    - **No-write-allocate (write-around)**: The data is written directly to main memory without loading the block into the cache. Typically paired with write-through, since write-through already ensures main memory stays up to date. However, this means that if the CPU later *reads* that address, it'll be a cache miss and the block gets fetched then.
...
**Cache thrashing** is a severe performance degradation phenomenon occurring when CPU cache lines are repeatedly evicted and re-loaded, causing the system to spend more time fetching data from slow main memory than executing instructions. It happens when a working set exceeds cache capacity or due to conflicting memory access patterns.
