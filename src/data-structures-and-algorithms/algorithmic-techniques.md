...
> [!NOTE]
> Max heaps are natively supported in Python 3.14 or later. For any earlier Python versions, to simulate a max heap, first build a new list `pq` from the elements of the input but negated using `-`, call `heapq.heapify(pq)`, and then negate each popped value to get back the original.

> [!NOTE]
> To simulate an indexed priority queue, wrap each item in your list as a tuple: `(key, element)`. If you are max heaps and are using a Python interpreter without native max heap native support, you must negate the key by `-`, and if the question involves tie-breakers, negate the element by `-`.
...
## QuickSelect (Hoare's Selection Algorithm) (TO LEARN)
