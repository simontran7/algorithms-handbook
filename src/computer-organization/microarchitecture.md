...
<img src="images/single-core-architecture.png" width="300" />
...
The communication channel comprised of a set of wires which as a whole connects the units so that they may transfer information in binary to another unit is called the **bus**. Typically, architectures have separate buses for sending different types of information between units:
...
Notice that in the serial execution of the fetch-decode-execute cycle, the fetch hardware ends up being idle until the next instruction starts. This is wasteful! While instruction 1 is being decoded/executed/written back, the fetch unit could already be fetching instruction 2, 3, etc.
